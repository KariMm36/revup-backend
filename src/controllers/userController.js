...
const { User, Job, Skill, SavedJob } = require('../models');
...
exports.toggleSaveJob = async (req, res, next) => {
  try {
    ...
    if (savedJobs.length > 0) {
      ...
      return res.status(200).json({ success: true, message: 'Job removed from saved jobs.' });
    } else {
...
exports.getSeekerProfile = async (req, res, next) => {
  try {
    ...
    // Privacy guard: only allow if seeker applied to one of this recruiter's jobs
    const company = await Company.findOne({ where: { recruiter_id: req.user.id } });
    if (!company) return res.status(403).json({ success: false, message: 'You do not have a company profile.' });
